...
clear all
d1f = 88.95; %mm
for test_num = 1:100000 % Run 10000 IK tests
    ...
    while (out_of_reach==true)
        gen_fail=true;
        while(gen_fail==true)
            input_ang = (-pi/2 + (pi/2+pi/2) .* rand(6,1)); % Generate 6 joint angles
    ...
    pass_or_fail = 1;
    error_r = abs(sum(sum(abs(Test_frame(1:3,1:3)\result_frame(1:3,1:3))-eye(3))));
    error_r_180 = abs(sum(sum(abs(Test_frame(1:3,1:3)\(result_frame(1:3,1:3)*rotz(pi)))-eye(3))));

    error_p = norm(Test_frame(1:3,4)-result_frame(1:3,4)); % calculate the error between Ik return frame and true frame
    if error_p> 2 || error_r>0.1 % 0.1 and 0.01 for loose testing.  2 0.1 for tight testing
        % Display failed test data
        Test_frame;
        result_frame;
        er_str = string(error_p);
    ...
    Test_details(test_num,1:22) = [time_passed,pass_or_fail,error_p,input_ang',Test_frame(1:3,4)',result_frame(1:3,4)',error_r,return_ang'];
...
average_error_position = mean(Passed_test_details(:,3))
average_error_rotation = mean(Passed_test_details(:,16))
...
plot3(Failed_test_details(:,13),Failed_test_details(:,14),Failed_test_details(:,15),'r.')
...
% input_ang = [-0.409456215730438	0.987975197717520	2.33913786250458	0.932931040552017	-1.04110975440021	-1.48513376214693];
